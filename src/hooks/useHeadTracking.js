 ...
 */
export function useHeadTracking(skeletonRoot, config = {}, isSpeaking = false) {
  ...
  useFrame(() => {
    ...
    const charForward = characterForward.current.set(0, 0, 1).applyQuaternion(spineWorldQuat);
    ...
    if (isInTrackingRange) {
      ...
      if (absAngle <= options.headOnlyAngle) {
        ...
        targetNeckYaw = extraAngle * Math.sign(clampedHorizontal) * options.neckYawFactor;
      }
    }
    ...
    trackingWeight.current += (targetWeight - trackingWeight.current) * options.lerpSpeed;
    
    // Smooth the ANGLES (not quaternions) - this gives stable results
    const lerp = options.lerpSpeed;
    smoothedAngles.current.headYaw += (targetHeadYaw - smoothedAngles.current.headYaw) * lerp;
    smoothedAngles.current.headPitch += (targetHeadPitch - smoothedAngles.current.headPitch) * lerp;
    smoothedAngles.current.neckYaw += (targetNeckYaw - smoothedAngles.current.neckYaw) * lerp;
    smoothedAngles.current.neckPitch += (targetNeckPitch - smoothedAngles.current.neckPitch) * lerp;
    ...
    const headLookRotation = tempQuat2.setFromEuler(
      new THREE.Euler(smoothedAngles.current.headPitch, smoothedAngles.current.headYaw, 0, "YXZ")
    );
    const trackingHeadQuat = restPose.current.head.clone().multiply(headLookRotation);
    
    // Blend between animation and tracking based on weight
    // weight = 0: pure animation, weight = 1: pure tracking
    head.quaternion.copy(animHeadQuat).slerp(trackingHeadQuat, trackingWeight.current);
    ...
    if (neck2 && animNeck2Quat) {
      ...
      const trackingNeck2Quat = restPose.current.neck2.clone().multiply(neck2LookRotation);
      neck2.quaternion.copy(animNeck2Quat).slerp(trackingNeck2Quat, trackingWeight.current);
    }
    ...
    if (neck1 && animNeck1Quat) {
      ...
      const trackingNeck1Quat = restPose.current.neck1.clone().multiply(neck1LookRotation);
      neck1.quaternion.copy(animNeck1Quat).slerp(trackingNeck1Quat, trackingWeight.current);
    }
  });
